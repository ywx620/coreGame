package org.coreMstchingGame
{
	...
	{
		private static const INIT_SPEED:Number=4;
		...
		private var moveRects:Array;
		private var _gameMap:GameMap;
		private var max:Number;
		private var mapClone:Array;
		private var mapHead:Rectangle;
...
//				else if(i==8)		rect.setRect(Const.SIZE,Const.SIZE,1,Const.COLORS[1]);
		...
		private function getMapRectByRect(rect:Rectangle):Rectangle
		{
		...
		private function getCloneRectByRect(rect:Rectangle):Rectangle
		{
		...
		/**替换地图*/
		private function replaceMap():void
		{
		...
		/**查找需要移动的方块*/
		private function pushMoveRect():void
		{
			max=map.length-1;
			moveRects.length=0;
			mapHead=null;
		...
		/**增加新的方块*/
		private function addNewRect():void
		{
			var seat:Point=new Point(map[0].x,map[0].y)
			var rect:Rectangle;
			var length:int=searchs.length;
			searchs.length=0;
			var interval:Number=Const.INTERVAL;
			//把原来搜索数组清空存入需要新加的方块
			for(var i:int=0;i<length;i++){
				rect=new Rectangle;
				var index:int=Math.ceil(Const.COLOR_NUM*Math.random());
				var color:uint=Const.COLORS[index];
				rect.setRect(Const.SIZE,Const.SIZE,index,color);
				//rect.ynum=rect.xnum=-1;
				rect.x=seat.x;
				rect.y=seat.y-rect.height-i*(rect.height+interval);
				searchs.push(rect);
				_gameMap.addChild(rect);
			}
		}
